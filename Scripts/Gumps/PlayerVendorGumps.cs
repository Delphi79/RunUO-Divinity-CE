		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 )
			{
				...
				else
				{
                    ...
                    // move this to allow items to go on the ground
                    // otherwise people use vendors as secure storage because you cant buy the bag because its too heavy
                    if (!from.PlaceInBackpack(m_VI.Item))
                    {
                        m_Vendor.SayTo( from, 503204 ); // You do not have room in your backpack for this.
                        m_VI.Item.MoveToWorld( from.Location, from.Map );
                    }
				}
			}
			else
