...
namespace Server.Gumps
{
	public class RunebookGump : Gump
	{
		...
		private void AddBackground()
		{
			...
			// Charges
			AddHtml( 161, 40, 100, 80, "<big>Charges:</big>", false, false);
			AddHtml( 220, 40, 30, 18, String.Format("<big>{0}</big>", m_Book.CurCharges.ToString()), false, false);

			// Max charges
			AddHtml( 300, 40, 100, 18, "<big>Max Charges:</big>", false, false);
			AddHtml( 400, 40, 30, 18, String.Format("<big>{0}</big>", m_Book.MaxCharges.ToString()), false, false);
		}
		...
		private void AddIndex()
		{
			...
			AddHtml( 158, 22, 100, 18, "<big>Rename book</big>", false, false);
			...
			for ( int i = 0; i < 16; ++i )
			{
				...
				// Description label
				AddHtml( 145 + ((i / 8) * 160), 60 + ((i % 8) * 15), 130, 17, String.Format( "<big>{0}</big>", desc ), false, false);
			}
		...
		private void AddDetails( int index, int half )
		{
			...
			if ( index < m_Book.Entries.Count )
			{
				...
				{
					AddHtml( 135 + (half * 160), 80, 100, 18, String.Format("<big>{0}o{1}'{2}</big>", yLat, yMins, ySouth ? "S" : "N"), false, false);
					AddHtml( 135 + (half * 160), 95, 100, 18, String.Format("<big>{0}o{1}'{2}</big>", xLong, xMins, xEast ? "E" : "W"), false, false);
				...
				AddHtml( 150 + (half * 160), 115, 100, 18, "<big>Drop rune</big>", false, false);
				...
				if ( e != m_Book.Default )
				{
					...
					AddHtml( 175 + (half * 140), 15, 100, 18, "<big>Set default</big>", false, false);
			...
			AddHtml( 145 + (half * 160), 60, 130, 17, String.Format( "<big>{0}</big>", desc ), false, false);
