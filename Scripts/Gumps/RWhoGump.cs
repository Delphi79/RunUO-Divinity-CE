...
{
	public class rWhoGump : Gump
	{
		public static void Initialize()
		{
			CommandSystem.Register( "rWho", AccessLevel.Counselor, new CommandEventHandler( rWhoList_OnCommand ) );
		}

		[Usage( "rWho [filter]" )]
		[Description( "Lists all connected clients with resource gathering skills. Optionally filters results by name." )]
		private static void rWhoList_OnCommand( CommandEventArgs e )
		{
			e.Mobile.SendGump( new rWhoGump( e.Mobile, e.ArgString ) );
		}
		...
		public rWhoGump( Mobile owner, string filter ) : this( owner, BuildList( owner, filter ), 0 )
		{
		}

		public rWhoGump( Mobile owner, List<Mobile> list, int page ) : base( GumpOffsetX, GumpOffsetY )
		{
			owner.CloseGump( typeof( rWhoGump ) );
		...
		public static List<Mobile> BuildList( Mobile owner, string filter )
		{
			...
			for ( int i = 0; i < states.Count; ++i )
			{
				...
				if ( m != null && m.AccessLevel == AccessLevel.Player )
				{
					...
					if ( m.Skills[SkillName.Mining].Value > 65 || m.Skills[SkillName.Fishing].Value > 65 || m.Skills[SkillName.Lumberjacking].Value > 65 )
					{
						list.Add( m );
					}
				}
		...
		public void Initialize( int page )
		{
			...
			AddLabel( x + TextOffsetX, y, TextHue, String.Format( "Page {0} of {1} ({2})", page+1, (m_Mobiles.Count + EntryCount - 1) / EntryCount, Server.Admin.AdminNetwork.GetNumOnline() ) );
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				{
					if ( m_Page > 0 )
						from.SendGump( new rWhoGump( from, m_Mobiles, m_Page - 1 ) );
				...
				default:
				{
					...
					if ( index >= 0 && index < m_Mobiles.Count )
					{
						...
						if ( m.Deleted )
						{
							...
							from.SendGump( new rWhoGump( from, m_Mobiles, m_Page ) );
						}
