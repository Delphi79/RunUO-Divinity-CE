...
namespace Server.Gumps
{
	...
	public class AdminGump : Gump
	{
			...
			switch ( pageType )
			{
				...
				case AdminGumpPage.Clients:
				{
					if ( m_List == null )
					{
						...
						ArrayList m_Remove = new ArrayList();

						for ( int i = 0; i < m_List.Count; i++ )
						{
							NetState ns = m_List[i] as NetState;

							if ( ns.Mobile != null && ns.Mobile.AccessLevel > from.AccessLevel && ns.Mobile.Hidden == true )
							{
								m_Remove.Add( m_List[i] );
							}
						}
						for ( int t = 0; t < m_Remove.Count; t++ )
						{
							m_List.Remove( m_Remove[t] );
						}
					...
					for ( int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index )
					{
						...
						if ( ns == null )
							continue;
						//else if ( ns.Mobile != null && ns.Mobile.AccessLevel > from.AccessLevel && from.AccessLevel != AccessLevel.Owner )
						//	continue;
						...
						if ( ( a != null || m != null ) && a.AccessLevel <= from.AccessLevel && m.AccessLevel <= from.AccessLevel )
				...
				case AdminGumpPage.Accounts_Shared:
				{
					...
					for ( int i = 0; i < m_List.Count; i++ )
					{
						Account ac = m_List[i] as Account;

						if ( ac != null && ac.AccessLevel > from.AccessLevel )
						{
							m_Remove.Add( m_List[i] );
						}
					}
				...
				case AdminGumpPage.Accounts:
				{
					...
					for ( int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index )
					{
						...
						if ( a == null )
							continue;
						//else if ( a.AccessLevel >= from.AccessLevel && from.AccessLevel != AccessLevel.Owner )
						//	continue;
						...
						{
							if ( accessLevel <= from.AccessLevel || from.AccessLevel >= AccessLevel.Developer )
				...
				case AdminGumpPage.Firewall:
				{
					...
					for ( int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index )
					{
						...
						if ( obj != null && !(obj is Firewall.IFirewallEntry ) )
							break;
		...
		public void AddAccountHeader()
		{
			...
			if ( m_From != null && m_From.AccessLevel >= AccessLevel.Developer )
