...
namespace Server.SkillHandlers
{
    public class AnimalTaming
    {
        ...
        {
            private bool m_SkillTimeSet = false;
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is Mobile)
                {
                    if (targeted is BaseCreature)
                    {
                        ...
                        else if (from.Followers + creature.ControlSlots > from.FollowersMax && from.FollowersMax > 0)
                        {
                        ...
                        else if (CheckMastery(from, creature) || from.Skills[SkillName.AnimalTaming].Value >= creature.MinTameSkill * 1.2 - 25.0)
                        {
                            ...
                            else if ((creature is Dragon || creature is Drake || creature is Nightmare || creature is WhiteWyrm) && 0.90 >= Utility.RandomDouble())
                            {
                            ...
                            else
                            {
                                ...
                                from.NextSkillTime = DateTime.Now + TimeSpan.FromSeconds(3.5);
                                m_SkillTimeSet = true;
                            }
                        }
                        else
            ...
            private class InternalTimer : Timer
            {
                ...
                protected override void OnTick()
                {
                    ...
                    {
                        m_Tamer.NextSkillTime = DateTime.Now + TimeSpan.FromSeconds(3.5);
                    ...
                    else
                    {
                        ...
                        */
                        double skill = m_Creature.MinTameSkill;

                        if (skill > 0 && CheckMastery(m_Tamer, m_Creature))
                            skill = 0; // 50% at 0.0?

                        if ( skill > 90 )
                            skill -= (skill - 90) / 2; // scale from -0 at 90 to -5 at 100

                        bool success;
                        if (alreadyOwned)
                            success = m_Tamer.CheckTargetSkill( SkillName.AnimalTaming, m_Creature, skill - 40, skill );
                        else
                            success = m_Tamer.CheckTargetSkill( SkillName.AnimalTaming, m_Creature, skill, skill + 40.0 );
                        ...
                        if (success)
                        {
                            ...
                            int val = (int)((m_Tamer.Skills[SkillName.AnimalTaming].Value - skill) / 100.0 * BaseCreature.MaxLoyalty);

                            if ( val < 25 )
                                val = 25;
                            if ( val > BaseCreature.MaxLoyalty )
                                val = BaseCreature.MaxLoyalty;

                            m_Creature.Loyalty = val;
                        }
                        else
                        {
