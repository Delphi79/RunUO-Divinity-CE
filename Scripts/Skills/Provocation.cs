        ...
        private class InternalSecondTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is Mobile)
                {
                    ...
                    else if (m_Creature != targ && m_Creature != null)
                    {
                        if (from == targ)
                        {
                        ...
                        else if (from.CanBeHarmful(m_Creature, true) && from.CanBeHarmful(targ, true))
                        {
                            ...
                            else
                            {
                                ...
                                if ( !from.CheckTargetSkill(SkillName.Provocation, targ, m_Creature.BardLevel, ( m_Creature.BardLevel + 25 ) ))
                                {
                                    from.SendLocalizedMessage(501599); // Your music fails to incite enough anger.
                                    m_Instrument.PlayInstrumentBadly(from);
                                    m_Instrument.ConsumeUse(from);

                                    // inserted this. maybe it's buggy, who knows.
