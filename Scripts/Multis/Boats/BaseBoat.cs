...
namespace Server.Multis
{
	...
	public abstract class BaseBoat : BaseMulti
	{
		...
		public bool LowerAnchor( Mobile from, bool message )
		{
			if ( CheckDecay() )
				return false;
            ...
            if ( from != null )
                pack = from.Backpack;
            if (pack == null || (from.AccessLevel <= AccessLevel.Counselor && (m_SPlank == null || !Key.ContainsKey(pack, m_SPlank.KeyValue)) && (m_PPlank == null || !Key.ContainsKey(pack, m_PPlank.KeyValue))))
            {
                if (message && m_TillerMan != null)
                    m_TillerMan.PublicOverheadMessage(MessageType.Regular, 0x3b2, true, "Ar, you don't have the key!");
                return false;
            }
		...
		public bool RaiseAnchor( Mobile from, bool message )
		{
			if ( CheckDecay() )
				return false;
				...
				for ( int i = 0; i < e.Keywords.Length; ++i )
				{
					...
					if ( keyword >= 0x42 && keyword <= 0x6B )
					{
						switch ( keyword )
						{
							...
							case 0x6A: LowerAnchor( from, true ); break;
							case 0x6B: RaiseAnchor( from, true ); break;
		...
		public bool Turn( int offset, bool message )
		{
            ...
            // can only move forward when the plank is open
            if ( (m_PPlank != null && m_PPlank.IsOpen) || (m_SPlank != null && m_SPlank.IsOpen) )
            {
                if (message && m_TillerMan != null)
                    m_TillerMan.PublicOverheadMessage(MessageType.Regular, 0x3B2, true, "Ar, can't move with the plank out!");
		...
		public bool Move( Direction dir, int speed, bool message )
		{
            ...
            // can only move forward when the plank is open
            if ( Direction != dir && ((m_PPlank != null && m_PPlank.IsOpen) || (m_SPlank != null && m_SPlank.IsOpen)) )
            {
                if ( message && m_TillerMan != null )
					m_TillerMan.PublicOverheadMessage(MessageType.Regular, 0x3B2, true, "Ar, can't move with the plank out!" );
