...
namespace Server.Multis
{
	...
	public class HousePlacement
	{
		...
		public static HousePlacementResult Check( Mobile from, int multiID, Point3D center, out ArrayList toMove )
		{
            ...
            // this check has been moved because of some problems with items when placing a house over them.
            // plus now staff can know if a spot is valid or not (the house will place still, but they will get the error message
			...
			for ( int x = 0; x < mcl.Width; ++x )
			{
				for ( int y = 0; y < mcl.Height; ++y )
				{
					...
					if ( hasFoundation )
					{
						bool isTower = (multiID & 0x3FFF) == 0x7A || (multiID & 0x3FFF) == 0x7B;
						int YardWidth = isTower ? 6 : 1;

						for ( int xOffset = -YardWidth; xOffset <= YardWidth; ++xOffset )
						{
