...
namespace Server.Multis
{
	public abstract class BaseHouse : BaseMulti
	{
		...
		{
			get{ return m_Condemned; }
			set{ m_Condemned = value; }
		}

        public static readonly TimeSpan BanRemovalTimer = TimeSpan.FromHours( 24.0 );
		...
		public virtual DecayLevel DecayLevel
		{
			get
			{
                ...
                long decayTicks = DecayPeriod.Ticks;
				...
				int percent = (int) ((timeAfterRefresh.Ticks * 1000) / decayTicks);

				if ( m_AutoRefresh && percent >= 500 )
				{
					m_LastRefreshed = DateTime.Now;
		...
		public virtual void KillVendors()
		{
            ...
            // delete guildstones when houses collapse
            Item gs = this.FindGuildstone();
		...
		public bool CheckAccessibility( Item item, Mobile from )
		{
            ...
            /* containers are accessible for everyone
		...
		public uint CreateKeys( Mobile m )
		{
			...
			if ( !IsAosRules )
			{
				Key packKey = new Key( KeyType );
				Key bankKey = new Key( KeyType );
		...
		public void Kick( Mobile from, Mobile targ )
		{
			...
			else
			{
                ...
                from.SendAsciiMessage( "{0} has been ejected from the house.", targ.Name );
        ...
        public class BanEntry
        {
            ...
            public DateTime Placed { get { return m_Placed; } }

            public BanEntry(Mobile m, DateTime placed)
            {
                m_Mobile = m;
                m_Placed = placed;
            }

            public BanEntry(Mobile m)
                : this(m, DateTime.Now)
            {
            }
        }
		...
		public void Ban( Mobile from, Mobile targ )
		{
			...
			{
				m_Bans.Add( targ, new BanEntry( targ ) );
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (bool) m_Condemned );

			writer.Write( (bool) m_AutoRefresh );
            ...
            foreach ( BanEntry entry in m_Bans.Values )
            {
                if ( entry.Placed + BanRemovalTimer < DateTime.Now || entry.Mobile.Deleted )
                    toBeRemoved.Add( entry.Mobile );
            }
            foreach ( Mobile m in toBeRemoved )
                m_Bans.Remove( m );

            writer.Write( m_Bans.Count );
            foreach ( BanEntry entry in m_Bans.Values )
            {
                writer.Write(entry.Mobile);
                writer.Write(entry.Placed);
            }
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 16:
				{
					m_Condemned = reader.ReadBool();
					goto case 15;
				}
				case 15:
				{
					m_AutoRefresh = reader.ReadBool();
					goto case 14;
				}
				case 14:
				{
				...
				case 0:
				{
                    ...
                    for (int i = 0; i < banCount; i++)
                    {
                        ...
                        if (m != null)
                            m_Bans.Add(m, new BanEntry(m, dt));
                    }
		...
		public int MaxSecures
		{
			get
			{
                // completely disable secures on divinity
                return 0;
		...
		public Hashtable Bans{ get{ return m_Bans; } set{ m_Bans = value; } }
        ...
        public ArrayList BanList
        {
            get
            {
                ...
                list.Sort(m_BanEntrySorter);

                return list;
            }
        }

        private static readonly BanEntrySorter m_BanEntrySorter = new BanEntrySorter();

        private class BanEntrySorter : IComparer
        {
            public int Compare(object a, object b)
            {
                BanEntry x = (BanEntry)a;
                BanEntry y = (BanEntry)b;

                if (x.Placed == y.Placed)
                    return 0;
                else if (x.Placed > y.Placed)
                    return 1;
                else
                    return -1;
            }
		...
		public override void OnAfterDelete()
		{
			...
			if ( m_Addons != null )
			{
				for ( int i = 0; i < m_Addons.Count; ++i )
				{
					...
					if ( item != null )
					{
                        if (item is Tent.TentChest && !item.Deleted && item.Map != Map.Internal && item.Location != Point3D.Zero)
                        {
                            for (int j = 0; j < item.Items.Count; j++)
                                ((Item)item.Items[j]).MoveToWorld(item.Location, item.Map);
                        }
        ...
        public void RemoveKeys(Mobile m)
        {
            ...
            if (keyValue == 0 && this is Tent && m_Addons != null)
            {
                foreach (Item i in m_Addons)
                {
                    if (i is Tent.TentChest)
                    {
                        keyValue = ((Tent.TentChest)i).KeyValue;
                        if (keyValue != 0)
                            break;
                    }
                }
		...
		public void ChangeLocks( Mobile m )
		{
            ...
            if ( m_Addons != null && this is Tent )
            {
                foreach (Item i in m_Addons)
                {
                    if (i is Tent.TentChest)
                        ((Tent.TentChest)i).KeyValue = keyValue;
                }
            }
		}
		...
		public bool IsBanned( Mobile m )
		{
            ...
            foreach ( BanEntry be in m_Bans.Values ) //for ( int i = 0; i < m_Bans.Count; ++i )
			{
                Mobile c = be.Mobile;//(Mobile)m_Bans[i];
		...
		public new bool IsSecure( Item item )
		{
            ...
            // remove secures from divinity completely
            return false;
		}
