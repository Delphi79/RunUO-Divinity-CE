...
namespace Server.Spells
{
	...
	public class SpellHelper
	{
		...
		public static readonly TimeSpan OldDamageDelay = TimeSpan.FromSeconds( 0.5 );
		...
		public static TimeSpan GetDamageDelayForSpell( Spell sp )
		{
			...
			if ( sp is EnergyBoltSpell )
				return TimeSpan.FromSeconds( 0.35 );
			else
		...
		public static bool ValidIndirectTarget( Mobile from, Mobile to )
		{
            ...
            if (to.Region is Engines.ConPVP.SafeZone)
                return false;
        ...
        private static bool[,] m_Rules = new bool[,]
			{
					/*T2A(Fel)		Ilshenar		Wind(Tram),	Wind(Fel),	Dungeons(Fel),	Solen(Tram),	Solen(Fel), CrystalCave(Malas),	Gauntlet(Malas),	Gauntlet(Ferry),	SafeZone,	Stronghold,		ChampionSpawn, Dungeons(Tokuno[Malas]) */
/* Recall From */	{ false, 		true,			true,		true,		true,			true,			false,		false,				false,				false,				true,		true,			true,			true	 },
/* Recall To */		{ false,	    	false,			false,		false,		true,			false,			false,		false,				false,				false,				false,		false,			false,			false	 },
/* Gate From */		{ false,		    false,			false,		false,		true,			false,			false,		false,				false,				false,				false,		false,			false,			false	 },
/* Gate To */		{ false, 		false,			false,		false,		true,			false,			false,		false,				false,				false,				false,		false,			false,			false	 },
/* Mark In */		{ true,	    	false,			false,		false,		true,			false,			false,		false,				false,				false,				false,		false,			false,			false	 },
/* Tele From */		{ true,			true,			true,		true,		true,			true,			true,		false,				true,				true,				true,		false,			true,			true	 },
/* Tele To */		{ true,			true,			true,		true,		true,			true,			true,		false,				true,				false,				false,		false, 			true,			true	 },
			};

        public static bool TravelInsideT2A(Mobile mob)
        {
            return !IsSafeZone(mob.Map, mob.Location) && IsAnyT2A(mob.Map, mob.Location);
        }

        public static bool TravelInsideT2A(Point3D location, Mobile mob)
        {
            return !IsSafeZone( mob.Map, location ) && !IsSafeZone( mob.Map, mob.Location ) &&
                IsAnyT2A( mob.Map, location ) && IsAnyT2A( mob.Map, mob.Location );
        }
        ...
        public static bool IsSafeZone(Map map, Point3D loc)
        {
            ...
            {
                if (m_TravelType == TravelCheckType.TeleportTo)
                {
                    PlayerMobile pm = m_TravelCaster as PlayerMobile;

                    // say they are not in a safe zone if they are trying to teleport during an event game
                    // if teleport is not allowed by the event game then they wont get this far
                    // if it is allowed, then we shouldnt block them here.
                    if( pm != null && pm.DuelPlayer != null && !pm.DuelPlayer.Eliminated &&
                        pm.DuelContext != null && pm.DuelContext.Started && !pm.DuelContext.Finished && 
                        pm.DuelContext.Arena != null && pm.DuelContext.Arena.Tournament != null && 
                        pm.DuelContext.Arena.Tournament.Tournament != null && 
                        pm.DuelContext.Arena.Tournament.Tournament.EventController != null )
                    {
                        return false;
                    }
                }
        ...
        public static bool IsT2ADungeon( Map map, Point3D loc )
        {
            ...
            if (reg != null && reg.IsPartOf(typeof(DungeonRegion)))
            {
		...
		public static bool IsFeluccaT2A( Map map, Point3D loc )
		{
            ...
            if (map == Map.Felucca && x >= 5120 && y >= 2304 && x < 6144 && y < 4096)
                return true;

            return IsT2ADungeon(map, loc);
		}
		...
		public static bool IsAnyT2A( Map map, Point3D loc )
		{
            ...
            if ((map == Map.Trammel || map == Map.Felucca) && x >= 5120 && y >= 2304 && x < 6144 && y < 4096)
                return true;
            return IsT2ADungeon(map, loc);
		}
		...
		public static bool CheckTown( Point3D loc, Mobile caster )
		{
            ...
            if (!CheckSafeZone(loc, caster))
            {
                caster.SendMessage("You cannot cast this in a safe zone!");
                return false;
            }
