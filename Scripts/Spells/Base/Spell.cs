...
namespace Server.Spells
{
	public abstract class Spell : ISpell
	{
		...
		private static bool SpellDelay = false;
		private static TimeSpan NextSpellDelay = TimeSpan.FromSeconds( 1.00 );
		//private TimeSpan NextSpellDelay { get{ GetCastDelay(); } }
        ...
        public virtual void OnCasterHurt(int damage)
        {
            if (IsCasting && Caster.Player)
            {
                ...
                if ( this is MagerySpell )
                    circle = (this.Scroll != null ? ((int)((MagerySpell)this).Circle) - 2 : (int)((MagerySpell)this).Circle);
                ...
                //if ( circle < 3 ) // basically gives no chance over circle 3, but a slight chance at circles 3,2,1
                //    disrupt = ( damage / ((double)(4 - circle )) ) < Utility.RandomDouble();
		...
		{
			if ( m_State == SpellState.Sequencing && o is Item )
		...
		{
            return true;//return m_Info.AllowTown;
		}
		...
		public virtual TimeSpan GetDisturbRecovery()
		{
			...
			double castdelay = GetCastDelay().TotalSeconds;

			if ( castdelay == 0 )
				return TimeSpan.FromSeconds( 1.0 );

			double delay = 1.0 - Math.Sqrt( (DateTime.Now - m_StartCastTime).TotalSeconds / castdelay );
		...
		public virtual double CastDelaySecondsPerTick { get { return 1.0; } }
		public virtual TimeSpan CastDelayMinimum { get { return TimeSpan.FromSeconds( 0.5 ); } }
		...
		public virtual TimeSpan GetCastDelay()
		{
			...
			TimeSpan circleDelay;
				circleDelay = TimeSpan.FromSeconds( (double)circle / 1.5 );

			//int delay = CastDelayBase + circleDelay + fcDelay;
			//TimeSpan delay = baseDelay + fcDelay;
			TimeSpan delay = baseDelay + circleDelay;
        ...
        public bool CheckHandsFree()
        {
            ...
            return SkillHandlers.Meditation.CheckOkayHolding( oneHanded ) && SkillHandlers.Meditation.CheckOkayHolding( twoHanded );
        }
