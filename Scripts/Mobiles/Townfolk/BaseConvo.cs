...
namespace Server.Mobiles
{
    ...
    public class NotorietyList : IStringList
    {
        ...
        {
            int val = (int)((pc.Karma + 10000.0) / 4000.0);
            if (val <= 0)
    ...
    public class BaseConvo : BaseCreature
    {
        ...
        public BaseConvo(Serial serial)
            : base(serial)
        {
        }
        ...
        public static string FixRegionName(string name)
        {
            System.Text.StringBuilder sb = new StringBuilder(name.ToLower());
        ...
        protected virtual void GetConvoFragments(ArrayList list)
        {
            if (this.Region is Regions.GuardedRegion)
            {
                ...
                try
                {
                    RegionFragment frg = (RegionFragment)Enum.Parse(typeof(RegionFragment), FixRegionName(this.Region.Name));

                    if (frg != RegionFragment.Wilderness)
                        list.Add((int)frg);
                }
                catch
                {
        ...
        {
            return base.HandlesOnSpeech(from) || (Enabled && from != null && from.Player && from.Alive && from.InRange( this, 3 ) && from != ControlMaster);
        }
        ...
        public override bool OnItemGiven(Mobile from, Item dropped)
        {
            Direction = GetDirectionTo(from);

            string itemName;

            if ( string.IsNullOrEmpty(itemName = dropped.BuildSingleClick()) )
                itemName = "something";

            Say( "Thou art giving me {0}?", itemName );

            if (base.OnItemGiven(from, dropped))
                return true;

            if (dropped is Gold)
            {
                if (dropped.Amount >= 400)
                    Say("'Tis a noble gift.");
                else
                    Say("Money is always welcome.");
            ...
            {
                string text = "I am not interested in this.";

                switch (Utility.Random(8))
                {
                    case 0: text = "Keep it! I have no need for it."; break;
                    case 1: text = "'Tis a shame I have no use for it. Keep it."; break;
                    case 2: text = "If only I needed it! I don't."; break;
                    case 3: text = "'Tis kind of thee, but I have no use for it."; break;
                    case 4: text = "Thank thee, but no."; break;
                    case 5: text = "I would like to accept thy gift, but I cannot. Thank thee."; break;
                    default: text = "I am not interested in this."; break;
                }
    ...
    public enum RegionFragment
    {
        ...
        buccaneersden = bucden,
        serpentshold = serphold,
        skarabrae = skara,
