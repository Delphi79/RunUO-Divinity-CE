	...
	public class PlayerVendor : Mobile
	{
		...
		public int ChargePerDay
		{
			...
			{ 
				if ( BaseHouse.NewVendorSystem )
				{
					return (int)( ( ChargePerRealWorldDay / 12 ) * 0.25 );
				}
				else
				{
					long total = 0;
					...
					return (int)( ( 20 + (total / 500) ) * 0.25 ); // reduce total charge by 25%
				}
			}
		}
		...
		private void OnItemGiven( Mobile from, Item item )
		{
			...
			if ( vi != null )
			{
                ...
                from.SendMessage( "Type in a price and description for {0} (ESC=not for sale)", item.BuildSingleClick() );
		...
		private class VendorPricePrompt : Prompt
		{
			...
			private void SetInfo( Mobile from, int price, string description )
			{
				...
				if ( price < 0 ) // Not for sale
				{
					...
					if ( item is Container || item is KeyRing )
					{
