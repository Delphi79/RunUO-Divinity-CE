...
namespace Server.Mobiles
{
    ...
    public abstract class BaseVendor : BaseConvo, IVendor
	{
		...
		public BaseVendor( string title ) : base( AIType.AI_Vendor, FightMode.Aggressor, 2, 1, 0.5, 0.75 )
		{
			LoadSBInfo();
		...
		public virtual void VendorBuy( Mobile from )
		{
			...
			if ( list.Count > 0 )
			{
                ...
                foreach (BuyItemState bis in list)
                {
                    ...
                    if (bis.ItemID > 300) // under 300 is probably an animal so dont send anything
                    {
                        if (loc > 500000)
        ...
        {
            public FakeOPL(Serial serial, int locNum)
                : base(0xD6)
            {
            ...
            private static System.Text.Encoding m_Encoding =System.Text.Encoding.Unicode;

            public FakeOPL(Serial serial, string desc)
                : base(0xD6)
            {
