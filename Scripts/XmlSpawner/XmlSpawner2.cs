...
namespace Server.Mobiles
{
	...
	public class XmlSpawner : Item, ISpawner
	{
		...
		public enum SpawnPositionType { Random, RowFill, ColFill, Perimeter, Player, Waypoint, RelXY, DeltaLocation, Location, Wet, Tiles, NoTiles }
		...
		private static string XmlConfigsDir = "SpawnerConfigs";  // default directory for loading .xml config files with LoadConfig
		...
		public ArrayList m_SpawnObjects = new ArrayList(); // List of objects to spawn
		...
		*/
		...
		{
			get { return (SpawnObject[])m_SpawnObjects.ToArray(typeof(SpawnObject)); }
			set
			{
		...
		*/
		...
		{
			if (from == null || from.Deleted || from.AccessLevel < AccessLevel.Seer || (m_SpawnerGump != null && SomeOneHasGumpOpen)) return;
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (m_SpawnObjects != null)
				...
				{
					string typename = ((SpawnObject)m_SpawnObjects[i]).TypeName;
					if (typename != null && typename.Length > 20)
					{
						typename = typename.Substring(0, 20);
					}

					list.Add(1060658 + (6 - nlist_items) + i, " {0}\t{1}", typename, ((SpawnObject)m_SpawnObjects[i]).SpawnedObjects.Count);
				}
...
public static void _TraceEnd(int index)
{
	   if(index < MaxTraces){
		...
		public static void Initialize()
		{
			...
			// standard commands
			CommandSystem.Register("XmlSpawnerShowAll", AccessLevel.Developer, new CommandEventHandler(ShowSpawnPoints_OnCommand));
			CommandSystem.Register("XmlSpawnerHideAll", AccessLevel.Developer, new CommandEventHandler(HideSpawnPoints_OnCommand));
			CommandSystem.Register("XmlSpawnerWipe", AccessLevel.Developer, new CommandEventHandler(Wipe_OnCommand));
			CommandSystem.Register("XmlSpawnerWipeAll", AccessLevel.Developer, new CommandEventHandler(WipeAll_OnCommand));
			...
			CommandSystem.Register("XmlShow", AccessLevel.Developer, new CommandEventHandler(ShowSpawnPoints_OnCommand));
			CommandSystem.Register("XmlHide", AccessLevel.Developer, new CommandEventHandler(HideSpawnPoints_OnCommand));

			CommandSystem.Register("XmlHome", AccessLevel.Seer, new CommandEventHandler(XmlHome_OnCommand));
			...
			CommandSystem.Register("XmlGet", AccessLevel.Seer, new CommandEventHandler(XmlGetValue_OnCommand));
			TargetCommands.Register(new XmlSetCommand());
			//CommandSystem.Register( "XmlSet", AccessLevel.Seer, new CommandEventHandler( XmlSetValue_OnCommand ) );
			...
			CommandSystem.Register("SmartStat", AccessLevel.Seer, new CommandEventHandler(SmartStat_OnCommand));
			CommandSystem.Register("XmlGo", AccessLevel.Seer, new CommandEventHandler(SpawnEditorGo_OnCommand));
			//CommandSystem.Register( "TagList", AccessLevel.Administrator, new CommandEventHandler( ShowTagList_OnCommand ) );

#if(TRACE)
			CommandSystem.Register( "XmlMake", AccessLevel.Developer, new CommandEventHandler( XmlMake_OnCommand ) );
			CommandSystem.Register( "XmlTrace", AccessLevel.Developer, new CommandEventHandler( XmlTrace_OnCommand ) );
			CommandSystem.Register( "XmlResetTrace", AccessLevel.Developer, new CommandEventHandler( XmlResetTrace_OnCommand ) );
#endif
		...
		public static void OptimalSmartSpawning_OnCommand(CommandEventArgs e)
		{
			...
			foreach (Item item in World.Items.Values)
			{
				if (item is XmlSpawner)
				{
					...
					for (int i = 0; i < nso; ++i)
					{
						SpawnObject so = spawner.m_SpawnObjects[i] as SpawnObject;
		...
		public int GetCurrentSequentialSpawnIndex(int sgroup)
		{
			...
			for (int j = 0; j < m_SpawnObjects.Count; j++)
			{
				if (((SpawnObject)m_SpawnObjects[j]).SubGroup == sgroup)
				{
					return (j);
				}
			}
		...
		private void SeqResetTo(int sgroup)
		{
			...
			if (spawnindex >= 0)
			{
				...
				SpawnObject s = (SpawnObject)m_SpawnObjects[spawnindex];
		...
		public void OnTick()
		{
			...
			if (!OnHold)
			{
				...
				{
					CheckTriggers((Mobile)null, null, false);
				}
		...
		public bool ClearSpawnedThisTick
		{
			set
			{
				...
				for (int i = 0; i < m_SpawnObjects.Count; i++)
				{
					SpawnObject sobj = (SpawnObject)m_SpawnObjects[i];
					if (sobj != null)
					{
		...
		public bool Spawn(bool smartspawn)
		{
			if (m_SpawnObjects != null && m_SpawnObjects.Count > 0 && (m_proximityActivated || CanFreeSpawn) && TODInRange)
			{
				...
				else
				{
					...
					if (Spawn(SpawnIndex, smartspawn, sobj.SpawnsPerTick) == true)
					{
						if (!smartspawn)
		...
		public bool Spawn(int index, bool smartspawn, int count, int packrange, Point3D packcoord, bool ignoreloopprotection)
		{
			if (m_SpawnObjects == null || index >= m_SpawnObjects.Count) return false;

			bool didspawn = false;

			SpawnObject so = (SpawnObject)m_SpawnObjects[index];
			...
			for (int k = 0; (k < nspawn) && (k + socnt < somax) && (k + scnt < MaxCount); k++)
			{
				...
				if (Spawn(index, smartspawn, packrange, packcoord, ignoreloopprotection))
				{
		...
		// spawn an individual entry by index up to count times
		public bool Spawn(int index, bool smartspawn, int count)
		{
			return Spawn(index, smartspawn, count, false);
		}

		// spawn an individual entry by index up to count times
		public bool Spawn(int index, bool smartspawn, int count, bool ignoreloopprotection)
		{
			return Spawn(index, smartspawn, count, -1, Point3D.Zero, ignoreloopprotection);
		}

		// spawn an individual entry by spawn object
		public void Spawn(string SpawnObjectTypeName, bool smartspawn, int packrange, Point3D packcoord)
		{
			if (m_SpawnObjects == null) return;
			for (int i = 0; i < m_SpawnObjects.Count; i++)
			{
				if (((SpawnObject)m_SpawnObjects[i]).TypeName.ToUpper() == SpawnObjectTypeName.ToUpper())
				{

					if (Spawn(i, smartspawn, packrange, packcoord))
					{
						RefreshNextSpawnTime((SpawnObject)m_SpawnObjects[i]);
					}
					break;
				}
				else
		...
		public void Spawn(string SpawnObjectTypeName, bool smartspawn)
		{
			Spawn(SpawnObjectTypeName, smartspawn, -1, Point3D.Zero);
		}

		// spawn an individual entry by index
		public bool Spawn(int index, bool smartspawn, int packrange, Point3D packcoord)
		{
			return Spawn(index, smartspawn, packrange, packcoord, false);
		}
		...
		public bool SpawnSubGroup(int sgroup)
		{
			return SpawnSubGroup(sgroup, false);
		}

		public bool SpawnSubGroup(int sgroup, bool smartspawn)
		{
			return SpawnSubGroup(sgroup, false, false);
		}

		public bool SpawnSubGroup(int sgroup, bool smartspawn, bool ignoreloopprotection)
		{
			...
			if (sgroup >= 0)
			{
				...
				for (int j = 0; j < m_SpawnObjects.Count; j++)
				{
					...
					if (so != null && so.SubGroup == sgroup)
					{
						...
						// get the SpawnsPerTick count and spawn up to that number
						bool success = Spawn(j, smartspawn, so.SpawnsPerTick, so.PackRange, packcoord, ignoreloopprotection);
		...
		public bool Respawn()
		{
			...
			{
				if (Spawn(false)) triedtospawn = true;
		...
		private class SubgroupSorter : IComparer
		{
			public int Compare(object x, object y)
			{
				SpawnObject a = x as SpawnObject;
				SpawnObject b = y as SpawnObject;
		...
		public static SpawnObject GetSpawnObject(XmlSpawner spawner, int sgroup)
		{
			...
			for (int i = 0; i < spawner.m_SpawnObjects.Count; i++)
			{
				...
				if (((SpawnObject)spawner.m_SpawnObjects[i]).SubGroup == sgroup)
				{
					return (SpawnObject)spawner.m_SpawnObjects[i];
				}
			}
			return null;
		}
		...
		public static object GetSpawned(XmlSpawner spawner, int sgroup)
		{
			...
			for (int i = 0; i < spawner.m_SpawnObjects.Count; i++)
			{
				...
				if (((SpawnObject)spawner.m_SpawnObjects[i]).SubGroup == sgroup)
				{
					...
					if (((SpawnObject)spawner.m_SpawnObjects[i]).SpawnedObjects.Count > 0)
					{
						return ((SpawnObject)spawner.m_SpawnObjects[i]).SpawnedObjects[0];
					}
				}
			}
			return null;
		...
		public static ArrayList GetSpawnedList(XmlSpawner spawner, int sgroup)
		{
			...
			for (int i = 0; i < spawner.m_SpawnObjects.Count; i++)
			{
				...
				if (((SpawnObject)spawner.m_SpawnObjects[i]).SubGroup == sgroup)
				{
					...
					if (((SpawnObject)spawner.m_SpawnObjects[i]).SpawnedObjects.Count > 0)
					{
						for (int j = 0; j < ((SpawnObject)spawner.m_SpawnObjects[i]).SpawnedObjects.Count; j++)
							newlist.Add(((SpawnObject)spawner.m_SpawnObjects[i]).SpawnedObjects[j]);
					}
				}
			}
			return newlist;
		...
		public bool HasSubGroups()
		{
			...
			for (int j = 0; j < m_SpawnObjects.Count; j++)
			{
				if (((SpawnObject)m_SpawnObjects[j]).SubGroup > 0) return true;
			}
		...
		private void RefreshNextSpawnTimes()
		{
			...
			if (m_SpawnObjects != null && m_SpawnObjects.Count > 0)
			{
				for (int i = 0; i < m_SpawnObjects.Count; i++)
				{
					SpawnObject so = m_SpawnObjects[i] as SpawnObject;
		...
		public Point2D GetRandomRegionPoint(Region r)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (o is Rectangle2D)
				{
					total += (FieldArray[i] = ((Rectangle2D)o).Width * ((Rectangle2D)o).Height);
				}
				else
					if (o is Rectangle3D)
					{
						total += (FieldArray[i] = ((Rectangle3D)o).Width * ((Rectangle3D)o).Height);
					}
			}
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (sum > rnd)
				{
					...
					if (o is Rectangle2D)
					{
						Rectangle2D r2d = (Rectangle2D)o;
						if (r2d.Width >= 0)
							x = r2d.X + Utility.Random(r2d.Width + 1);

						if (r2d.Height >= 0)
							y = r2d.Y + Utility.Random(r2d.Height + 1);
						...
						if (o is Rectangle3D)
						{
							Rectangle3D r3d = (Rectangle3D)o;
							if (r3d.Width >= 0)
								x = r3d.Start.X + Utility.Random(r3d.Width + 1);
							if (r3d.Height >= 0)
								y = r3d.Start.Y + Utility.Random(r3d.Height + 1);
							break;
						}
				}
			}
		...
		public Point3D GetSpawnPosition(bool requiresurface, int packrange, Point3D packcoord, List<SpawnPositionInfo> spawnpositioning, Mobile mob)
		{
			...
			if (includetilelist != null || excludetilelist != null || tileflag != TileFlag.None)
			{
				if (m_Region != null && HasRegionPoints(m_Region))
				{
					...
					if (positioning == SpawnPositionType.Random)
					{
						FindTileLocations(ref locations, this.Map, m_X, m_Y, m_Width, m_Height, includetilelist, excludetilelist, tileflag);
					}
			}
		...
		public int GetCreatureMax(int index)
		{
			...
			return ((SpawnObject)m_SpawnObjects[index]).MaxCount;
		}
		...
		public static object CreateObject(Type type, string[] typewordargs, bool requireconstructable, bool requireattachable)
		{
			...
			for (int i = 0; i < ctors.Length; ++i)
			{
				...
				if (!(requireconstructable && Add.IsConstructable(ctor, AccessLevel.Player)) && !(requireattachable && XmlAttach.IsAttachable(ctor)))
					continue;
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			{
				writer.Write(SpawnObjects.Length);
				for (int i = 0; i < SpawnObjects.Length; ++i)
				{
					// Write the spawns per tick value
					writer.Write(SpawnObjects[i].SpawnsPerTick);
				}
			}
			else
			{
			...
			if (SpawnObjects != null)
			{
				for (int i = 0; i < SpawnObjects.Length; ++i)
				{
					...
					writer.Write(SpawnObjects[i].PackRange);
				}
			}
			...
			if (SpawnObjects != null)
			{
				for (int i = 0; i < SpawnObjects.Length; ++i)
				{
					...
					writer.Write(SpawnObjects[i].Disabled);
				}
			}
			...
			if (SpawnObjects != null)
			{
				for (int i = 0; i < SpawnObjects.Length; ++i)
				{
					...
					writer.Write(SpawnObjects[i].RestrictKillsToSubgroup);
					// Write the clear on advance flag
					writer.Write(SpawnObjects[i].ClearOnAdvance);
					// Write the mindelay
					writer.Write(SpawnObjects[i].MinDelay);
					// Write the maxdelay
					writer.Write(SpawnObjects[i].MaxDelay);
					// write the next spawn time for the subgrop
					writer.WriteDeltaTime(SpawnObjects[i].NextSpawn);
			...
			if (SpawnObjects != null)
			{
				for (int i = 0; i < SpawnObjects.Length; ++i)
				{
					...
					writer.Write(SpawnObjects[i].RequireSurface);
				}
			}
			// Version 19
			...
			if (SpawnObjects != null)
			{
				for (int i = 0; i < SpawnObjects.Length; ++i)
				{
					...
					writer.Write(SpawnObjects[i].SubGroup);
					writer.Write(SpawnObjects[i].SequentialResetTime);
					writer.Write(SpawnObjects[i].SequentialResetTo);
					writer.Write(SpawnObjects[i].KillsNeeded);
				}
			}
			...
			// Write the spawn object list
			int nso = 0;
			if (m_SpawnObjects != null) nso = SpawnObjects.Length;
			writer.Write(nso);
			for (int i = 0; i < nso; ++i)
			{
				// Write the type and maximum count
				writer.Write((string)SpawnObjects[i].TypeName);
				writer.Write((int)SpawnObjects[i].ActualMaxCount);

				// Write the spawned object information
				writer.Write(SpawnObjects[i].SpawnedObjects.Count);
				for (int x = 0; x < SpawnObjects[i].SpawnedObjects.Count; ++x)
				{
					object o = SpawnObjects[i].SpawnedObjects[x];
