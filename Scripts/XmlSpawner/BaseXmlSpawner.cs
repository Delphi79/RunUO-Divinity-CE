...
namespace Server.Mobiles
{
    ...
    public class BaseXmlSpawner
    {
        ...
        private static ArrayList ProtectedPropertiesList = new ArrayList();
        ...
        {
            public ArrayList plist = new ArrayList();      // hold propertyinfo list
            public Type t;
        }
        ...
        public static bool IsValueKeyword(string str)
        {
            ...
            return valueKeywordHash.Contains(str);
        }
        ...
        public static bool IsValuemodKeyword(string str)
        {
            ...
            return valuemodKeywordHash.Contains(str);
        }

        public static bool IsSpecialItemKeyword(string typeName)
        {
            if (typeName == null || typeName.Length < 1 || !Char.IsUpper(typeName[0])) return false;

            if (itemKeywordHash == null) InitializeHash();

            return itemKeywordHash.Contains(typeName);
        }
        ...
        public static bool IsTypeKeyword(string typeName)
        {
            ...
            return (typeKeywordHash.Contains(typeName));
        }
        ...
        public static bool IsTypemodKeyword(string typeName)
        {
            ...
            return (typemodKeywordHash.Contains(typeName));
        }
        ...
        public static bool IsTypeOrItemKeyword(string typeName)
        {
            ...
            return (typeKeywordHash.Contains(typeName) || itemKeywordHash.Contains(typeName));
        }
        ...
        public class KeywordTag
        {
            ...
            public KeywordTag(string typename, XmlSpawner spawner, int type, TimeSpan delay, TimeSpan timeout, string condition, int gotogroup)
            {
                ...
                if (spawner != null && !spawner.Deleted)
                {
                    ...
                    {
                        spawner.m_KeywordTagList = new ArrayList();
                    }
                    ...
                    if (spawner.m_KeywordTagList.Count > 0)
                        Serial = ((KeywordTag)(spawner.m_KeywordTagList[spawner.m_KeywordTagList.Count - 1])).Serial + 1;
                    else
                        Serial = 0;
                    ...
                    if (m_Condition != null && m_Condition.Length > 0 && m_Spawner != null && m_Spawner.Running)
                    {
                        ...
                        if (TestItemProperty(m_Spawner, m_Spawner, m_Condition, trigmob, out status_str))
                        {
                            ...
                            if (m_Goto >= 0 && m_Spawner != null && !m_Spawner.Deleted)
                            {
                                ...
                                // spawn the subgroup
                                m_Spawner.SpawnSubGroup(m_Goto);
        ...
        public static string InternalSetValue(Mobile from, object o, PropertyInfo p, string value, bool shouldLog, int index)
        {
			...
			if ( from != null && o != null && o is PlayerMobile && ((PlayerMobile)o).AccessLevel > from.AccessLevel )
        ...
        public static ArrayList SearchPackListForItemType(Container pack, string targetName, ArrayList itemlist)
        {
            if (pack != null && !pack.Deleted && targetName != null && targetName.Length > 0)
            {
                Type targetType = null;
                try
                {
                    targetType = SpawnerType.GetType(targetName);
                }
                catch { }
        ...
        public static ArrayList FindItemListByType(Mobile m, string targetName, bool searchbank)
        {
        ...
        public static void AddToRecentItemSearchList(XmlSpawner spawner, Item target)
        {
            ...
            {
                spawner.RecentItemSearchList = new ArrayList();
            }
        ...
        public static void AddToRecentMobileSearchList(XmlSpawner spawner, Mobile target)
        {
            ...
            {
                spawner.RecentMobileSearchList = new ArrayList();
            }
        ...
        public static string[] ParseObjectArgs(string str)
        {
            ...
            if (arglist.Length > 0)
            {
                ...
                {
                    typeargs = ParseCommaArgs(itemtypestring.Substring(argstart), 10);
                }
                return (typeargs);
        ...
        public static string[] SplitString(string str, string separator)
        {
            ...
            int lastindex = 0;
            int index = 0;
            ArrayList strargs = new ArrayList();
            while (index >= 0)
            {
        ...
        public static void SendMusicToPlayers(string arglist, Mobile triggermob, object refobject, out string status_str)
        {
            ...
            if (musicstr.Length > 2)
            {
                ...
                try
                {
                    range = int.Parse(musicstr[2]);
                }
                catch { status_str = "bad range arg in MUSIC"; }
            }
                        ...
                        if (rangelist != null)
                        {
                            foreach (Mobile p in rangelist)
                            {
                                if (p is PlayerMobile)
                                {
                                    ...
                                    try
                                    {
                                        musicnumber = short.Parse(musicstr[1]);
                                    }
                                    catch { }
        ...
        public static void ApplyDamageToPlayers(string arglist, Mobile triggermob, object refobject, out string status_str)
        {
            ...
            if (str.Length > 7)
            {
                ...
                try
                {
                    range = int.Parse(str[7]);
                }
                catch { status_str = "bad range arg in DAMAGE"; }
            }
            if (str.Length > 8)
            {
                if (str[8].ToLower() == "playeronly") playeronly = true;
            }
            // get the damage arguments
            int damage = 0;
            int phys = 0;
            int fire = 0;
            int cold = 0;
            ...
            try
            {
                damage = int.Parse(str[1]);
                phys = int.Parse(str[2]);
                fire = int.Parse(str[3]);
                cold = int.Parse(str[4]);
                pois = int.Parse(str[5]);
                ener = int.Parse(str[6]);
            }
            catch { status_str = "bad damage args in DAMAGE"; }
