...
namespace Server.Commands
{
	public class CommandHandlers
	{
		public static void Initialize()
		{
            ...
            Register("G", AccessLevel.Player, new CommandEventHandler(GuildChat_OnCommand));
            ...
            Register("System", AccessLevel.GameMaster, new CommandEventHandler(SystemMessage_OnCommand));
        ...
        [Usage("g <text>")]
        [Description("Communicate with other members of your guild anywhere on the shard.")]
        private static void GuildChat_OnCommand(CommandEventArgs e)
        {
            PlayerMobile pm = e.Mobile as PlayerMobile;
            if (pm != null)
                pm.GuildMessage(e.ArgString);
        }
		...
		public static void StaffMessage_OnCommand( CommandEventArgs e )
		{
			...
			IRCBot.SendMessage(IRCBot.StaffChannel, String.Format("[{0}] {1}", e.Mobile.Name, e.ArgString ));
		}
        ...
        {
			if ( e.Mobile.AccessLevel >= AccessLevel.Administrator && e.Mobile.YellowHealthbar )
			{
				BroadcastMessage( AccessLevel.Player, 0x480, String.Format( "Global message from {0}:", e.Mobile.Name ) );
				BroadcastMessage( AccessLevel.Player, 0x480, e.ArgString );
			}
			else
			{
			...
			IRCBot.SendMessage(IRCBot.Channel, String.Format("[GLOBAL] {0}: {1}", e.Mobile.Name, e.ArgString ));
        }
        ...
        public static void SystemMessage_OnCommand(CommandEventArgs e)
        {
            World.Broadcast(0x34, true, e.ArgString);
        }

        public static void SystemMessage(string message)
        {
            World.Broadcast(0x34, true, message);
        }
        ...
        private static void Say_OnTarget(Mobile from, object targeted, object state)
        {
            ...
            {
                Mobile m = (Mobile)targeted;
                if (m.Player && m.AccessLevel >= from.AccessLevel && from.AccessLevel != AccessLevel.Administrator)
                {
