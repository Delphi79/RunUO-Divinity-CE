...
namespace Server.Mobiles
{
	public class Spawner : Item
	{
        ...
        {
            CommandSystem.Register("RespawnAll", AccessLevel.Administrator, new CommandEventHandler(RespawnAll_OnCommand));
        }
			...
			if ( type != null )
			{
				try
				{
					...
					else if ( o is Item )
					{
                        ...
                        TimeSpan delay;
                        if ( this.MinDelay < TimeSpan.FromMinutes( 5 ) )
                            delay = this.MinDelay;
                        else
                            delay = this.MinDelay - TimeSpan.FromMinutes( 4 );

                        if ( delay > item.DecayTime )
                            delay -= item.DecayTime;
                        else
                            delay = TimeSpan.Zero;

                        item.LastMoved = DateTime.Now + delay;
					}
				}
				catch
				{
