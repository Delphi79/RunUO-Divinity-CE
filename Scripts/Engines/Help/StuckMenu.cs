...
/*				// Trinsic
        ...
        private static StuckMenuEntry[] m_MurdererEntries = new StuckMenuEntry[]
			{
				// Bucs
				new StuckMenuEntry( 1011057, new Point3D[]
					{
						new Point3D( 2736, 2166, 0 ),
						new Point3D( 2667, 2084, 5 ),
						new Point3D( 2713, 2234, 0 )
					} ),

/*				// Chaos
				new StuckMenuEntry( 1011058, new Point3D[]
					{
						new Point3D( 1462, 844, 0 )
					} ),

                // nowhere
				new StuckMenuEntry( 1011058, new Point3D[]
					{
						new Point3D( 1001, 1002, 0 )
					} ) */
			};
...
*/
		...
		public StuckMenu( Mobile beholder, Mobile beheld, bool markUse ) : base( 150, 50 )
		{
            ...
            AddButton(50, 55 + 35, 208, 209, 1, GumpButtonType.Reply, 0);
            if (beheld.Kills < 5)
                AddHtml(75, 55 + 35, 335, 40, "Britain", false, false);
            else
                AddHtml(75, 55 + 35, 335, 40, "Buccaneer's Den", false, false);
...
/*				// 25% chance to get the worst one
                if (Utility.Random(4) == 0)
                    index = entries.Length - 1;
                else */

				index = Utility.Random( entries.Length );

                try { Teleport(entries[index]); }
                catch { }
			}
		}
		...
		private class TeleportTimer : Timer
		{
			...
			protected override void OnTick()
			{
				if ( DateTime.Now < m_End )
				{
                    ...
                    if ( m_Mobile.Spell is Spells.Spell )
                        ((Spells.Spell)m_Mobile.Spell).OnCasterHurt( 9999 );
                    if ( m_Mobile.Target != null )
                        m_Mobile.Target.Cancel( m_Mobile, Server.Targeting.TargetCancelType.Canceled );
				}
				else
				{
					m_Mobile.Frozen = false;
