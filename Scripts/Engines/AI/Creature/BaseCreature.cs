...
namespace Server.Mobiles
{
	...
	public class BaseCreature : Mobile, IHonorTarget
	{
        ...
        public virtual bool IsBondable { get { return (!Summoned && BondingEnabled); } }
		...
		public virtual bool IsEnemy( Mobile m )
		{
			...
			return ( 
                m_iTeam != c.m_iTeam || 
                ( (m_bSummoned || m_bControlled) != (c.m_bSummoned || c.m_bControlled) ) || 
                // in the case of EV and BS and maybe other weird fightmodes, they should attack anything
                ( FightMode != FightMode.Aggressor && FightMode != FightMode.Closest && FightMode != FightMode.None && m.GetType() != this.GetType() )
                /* || c.Combatant == this*/ );
		}
        ...
        public virtual bool CheckControlChance(Mobile m)
        {
            ...
            double chance = GetControlChance(m);
		...
		if (MinTameSkill >= 100)
		    attack = 0.50;
                else if (MinTameSkill >= 95)
                    attack = 0.075;
                else if (MinTameSkill >= 90)
                    attack = 0.05;
                else if (MinTameSkill >= 85)
                    attack = 0.01;
        ...
        {
            int pets = 1;
            double totalMinTameSkill = this.MinTameSkill;
            double chance = ((((double)this.Loyalty) + 1) / ((double)MaxLoyalty)) * 0.25 + 0.80; // can be up to 105%
            ...
            foreach (Mobile mob in eable)
            {
                if (mob != this)
                {
                    ...
                    if (bc != null && bc.Controlled && bc.ControlMaster == m)
                    {
                        if (bc.MinTameSkill > 90)
                        {
                            pets++;
                            totalMinTameSkill += bc.MinTameSkill;
                        }
                    }
                }
            }
            ...
            if (pets < 2)
                ...
                return chance - (0.05 * (pets-1));
        }

		public virtual bool CanBeControlledBy( Mobile m )
		{
            return true;//return ( GetControlChance( m ) > 0.0 );
		}
        ...
        private string m_CorpseName = null;
		...
		public override void OnDamage( int amount, Mobile from, bool willKill )
		{
            ...
            if (!willKill)
            {
                ...
                else if (Body.IsHuman && Utility.Random(10) == 0)
                {
                    if (amount <= 1)
                    {
                        string[] hitStr = { "Ha! Thou art inept!", "Thou didst miss, fool!", "Thy aim is bad...", "Surely thou canst do better than that blow!", "Thou dost hit only air!", "Thou art no match for me!" };
                        this.Say(hitStr[Utility.Random(hitStr.Length)]);
                    }
                    else if (amount <= 5)
                    {
                        string[] hitStr = { "Ouch! Thou didst scratch me!", "Barely a flesh wound. Canst thou not do better?", "Pfft, thou fightest badly.", "Surely thou canst hit harder than that!", "A bare touch... Thou dost not wield thy weapon well!" };
                        this.Say(hitStr[Utility.Random(hitStr.Length)]);
                    }
                    else
                    {
                        string[] hitStr = { "Ouch! A touch indeed!", "'Twill take more than that to kill me!", "Ow, thou didst get past my defenses!", "Away with thee, scum!", "Oof! That didst hurt!", "Aaah! I do bleed badly...", "A good blow on thy part... but not enough!" };
                        this.Say(hitStr[Utility.Random(hitStr.Length)]);
                    }
                }
            }
		...
		public const int OldRangePerception = 12;
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (double)m_bBardLevel );
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( version >= 17 )
			{
				...
				m_bBardLevel = reader.ReadDouble();
			}
        ...
        public virtual bool OnItemGiven(Mobile from, Item item)
        {
            return false;
        }
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			if ( CheckFeed( from, dropped ) )
				...
				return true;
            else if ( OnItemGiven( from, dropped ) )
                return true;
		...
		#region Penthus Edit2
		[CommandProperty( AccessLevel.GameMaster )]
		public bool BardImmune
		{
			get
			{
				return m_bBardImmune;
			}
			set
			{
				m_bBardImmune = value;
        ...
        public virtual double AutoDispelChance { get { return 0.20; } } // Changed from .25
		...
		public virtual void OnGaveMeleeAttack( Mobile defender )
		{
			...
			if( AutoDispel && defender is BaseCreature && ((BaseCreature)defender).IsDispellable && ( Math.Abs( this.Location.Z - defender.Location.Z ) < 15 ) && AutoDispelChance > Utility.RandomDouble() )
				Dispel( defender );
		}
			...
			if ( ( bPlayerOnly && m.Player ) ||  !bPlayerOnly )
			{
				switch( acqType )
				{
                    case FightMode.Strongest:
                        return m.Str; //returns strongest mobile
		...
		public void SetSkill( SkillName name, double min, double max )
		{
			...
			if ( Skills[name].Base > Skills[name].Cap ) 
				Skills[name].Base = Skills[name].Cap;

            if ( name == SkillName.Magery )
            {
                if ( Skills[SkillName.EvalInt].BaseFixedPoint < minFixed )
                    Skills[SkillName.EvalInt].BaseFixedPoint = Utility.RandomMinMax( minFixed, maxFixed );

                if ( Skills[SkillName.Meditation].BaseFixedPoint < minFixed )
                    Skills[SkillName.Meditation].BaseFixedPoint = Utility.RandomMinMax(minFixed, maxFixed);
            }
            else if ( name == SkillName.Tactics )
            {
                if ( Skills[SkillName.Anatomy].BaseFixedPoint < minFixed )
                    Skills[SkillName.Anatomy].BaseFixedPoint = Utility.RandomMinMax( minFixed, maxFixed );
            }
		}
        ...
        public override string ApplyNameSuffix(string suffix)
        {
            ...
            if (Controlled && Commandable)
            {
                if (suffix.Length == 0)
                    ...
                    suffix = String.Concat(suffix, " (tame)");
            }
		...
		public override bool OnBeforeDeath()
		{
            ...
            {
                if (Body.IsHuman && Utility.Random(10) == 0)
                {
                    if (this is BaseVendor)
                    {
                        string[] deathStr = { "Whatever shall my family do without me?", "I hope the guards catch thee, scum!", "Ooh... that doth hurt.", "Must I die?", "Curse thee!", "I shall be avenged...", "I shall haunt thee for this deed!", "I spit on thee..." };
                        this.Say(deathStr[Utility.Random(deathStr.Length)]);
                    }
                    else
                    {
                        string[] deathStr = { "I shall be avenged!", "NOOooo!", "I... I...", "No, I don't want to die...", "Argh! I am slain!", "Must stay... on feet...", "Oooh, that doth hurt...", "Am I dying?" };
                        this.Say(deathStr[Utility.Random(deathStr.Length)]);
                    }
                }
            }
			...
			else
			{
				if ( !Summoned && !m_NoKillAwards )
				{
					...
					for ( int i = 0; i < list.Count; ++i )
					{
                        ...
                        XmlQuest.RegisterKill(this, ds.m_Mobile);// modification to support XmlQuest Killtasks
		...
		public virtual bool Rummage()
		{
			...
			for ( int i = 0; i < items.Count; ++i )
			{
				...
				if ( !rejected && Drop( this, new Point3D( -1, -1, 0 ) ) )
				{
                    ...
                    string iname = item.BuildSingleClick();
                    string cname = toRummage.BuildSingleClick();
                    if ( string.IsNullOrEmpty( iname ) )
                        iname = "an item";
                    if ( string.IsNullOrEmpty( cname ) )
                        cname = "a corpse";
                    Emote( "rummages through {0} and takes {1}", cname, iname );
					return true;
				}
			}
