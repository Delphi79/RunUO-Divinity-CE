	...
	public abstract class BaseAI
	{
					...
					for( int i = 0; i < keywords.Length; ++i )
					{
						...
						switch( keyword )
						{
							...
							case 0x16D: // *release
							{
								...
								if( ( WasNamed( speech ) || Insensitive.StartsWith( speech, "all " ) ) && m_Mobile.CheckControlChance( e.Mobile ) )
								{
									if( !m_Mobile.Summoned )
									{
		...
		public virtual void OnActionChanged()
		{
			switch( Action )
			{
				...
				m_Mobile.Combatant = null;
				m_Mobile.CurrentSpeed = m_Mobile.ActiveSpeed;
				m_NextStopGuard = DateTime.Now + TimeSpan.Zero;
				m_Mobile.CurrentSpeed = m_Mobile.ActiveSpeed;
				break;
		...
		public virtual bool AcquireFocusMob( int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe )
		{
			...
			{
				if( m_Mobile.ControlTarget == null || m_Mobile.ControlTarget.Deleted || !m_Mobile.ControlTarget.Alive || m_Mobile.ControlTarget.IsDeadBondedPet || !m_Mobile.InRange( m_Mobile.ControlTarget, m_Mobile.RangePerception * 2 ) || !m_Mobile.CanSee( m_Mobile.ControlTarget ) )
				{
					m_Mobile.FocusMob = null;
					return false;
				}
			...
			if( map != null )
			{
				...
				foreach( Mobile m in eable )
				{
					...
					if ( m_Mobile.Controlled && m_Mobile.Summoned && m_Mobile.SummonMaster != null )
					{
