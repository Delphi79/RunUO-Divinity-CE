	...
	public class DuelContext
	{
		...
		public void Refresh( Mobile mob, Container cont )
		{
			if ( !mob.Alive )
			{
				...
				if ( robe != null && robe.Hue == 0 && !robe.IsMagic )
					robe.Delete();
		...
		{
            if (us.NetState != null && them.NetState != null)
            {
                if (us.NetState.Address.Equals(them.NetState.Address))
                    return;
            }
		...
		private static void EventSink_Speech( SpeechEventArgs e )
		{
			...
			if ( Insensitive.Contains( e.Speech, "i wish to duel" ) )
			{
				...
				else if ( pm.Map != Map.Felucca ) // only allow duels to be started from felucca
				{
					return;
				}
		...
		public static void Debuff( Mobile mob )
		{
			...
			//TransformationSpell.RemoveContext( mob, true );
			AnimalForm.RemoveContext( mob, true );

			if( DisguiseGump.IsDisguised( mob ) )
				DisguiseGump.OnDisguiseExpire( mob );
				...
				for ( int j = 0; j < p.Players.Length; ++j )
				{
					...
					else
					{
                        ...
                        if (pm != null)
                        {
                            if (pm.TempStatLoss.Count > 0)
                            {
                                pm.TempStatLoss[0].Pause();
                                pm.TempStatLoss[0].Stop();
                            }
                        }
					}
				}
		...
		public string ValidateStart()
		{
			...
			for ( int i = 0; i < m_Participants.Count; ++i )
			{
				...
				{
					DuelPlayer dp = p.Players[j];
					PlayerMobile pm = (PlayerMobile)dp.Mobile;

					if ( dp == null )
						return "a slot is empty";

					if ( pm != null && pm.Map != Map.Felucca && m_Tournament == null )
						return "all players are not in Felucca";
