...
namespace Server.Engines.ConPVP
{
	...
	public sealed class CTFFlag : Item
	{
		...
		public override string DefaultName
		{
			get { return "what's left of the flag"; }
		}
		...
		private void Countdown_OnTick()
		{
			...
			switch ( m_ReturnCount )
			{
				...
				{
					if ( owner != null )
						owner.SendMessage( 0x26, "You have {0} {1} to capture the flag!", m_ReturnCount, m_ReturnCount == 1 ? "second" : "seconds" );
				...
				case 0:
				{
					if ( owner != null )
					{
						owner.SendMessage( 0x26, "You have taken too long to capture the flag!" );
						owner.Kill();
					}
					...
					if ( m_TeamInfo != null && m_TeamInfo.Game != null )
						m_TeamInfo.Game.Alert( "The {0} flag has been returned.", m_TeamInfo.Name );
		...
		private void Flag_OnTarget( Mobile from, object obj )
		{
			...
			if ( obj is CTFFlag )
			{
				if ( obj == useTeam.Flag )
				{
					from.LocalOverheadMessage( MessageType.Regular, 0x59, false, "You captured the flag!" );
					m_TeamInfo.Game.Alert( "{0} captured the {1} flag!", from.Name, ourTeam.Name );
					...
					if ( playerInfo != null )
					{
						...
						if ( teamFlag.m_Fragger != null && DateTime.Now < ( teamFlag.m_FragTime + TimeSpan.FromSeconds( 5.0 ) ) )
						{
						...
						if ( teamFlag.m_Returner != null && DateTime.Now < ( teamFlag.m_ReturnTime + TimeSpan.FromSeconds( 5.0 ) ) )
						{
				...
				{
					from.LocalOverheadMessage( MessageType.Regular, 0x26, false, "That is not my flag" );
				}
			}
			else if ( obj is Mobile )
			{
				...
				{
					passTo.LocalOverheadMessage( MessageType.Regular, 0x59, false, String.Format( "{0} has passed you the flag!", from.Name ) );
				}
				else
				{
					from.LocalOverheadMessage( MessageType.Regular, 0x26, false, "I can't pass to them." );
	...
	public sealed class CTFController : EventController
	{
		...
		public override string DefaultName { get { return "CTF Controller"; } }
	...
	public sealed class CTFGame : EventGame
	{
		...
		private void Finish_Callback()
		{
            ...
            for (int i = 0; i < teams.Count; ++i)
            {
                ...
                CTFPlayerInfo leader = ((CTFTeamInfo)teams[i]).Leader;

                foreach (CTFPlayerInfo pl in ((CTFTeamInfo)teams[i]).Players.Values)
                {
                    ...
                    item.Name = String.Format("{0}, {1} team", item.Name, ((CTFTeamInfo)teams[i]).Name.ToLower());
