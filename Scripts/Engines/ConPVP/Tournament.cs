...
namespace Server.Engines.ConPVP
{
	...
	public class TournamentController : Item
	{
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( from.AccessLevel >= AccessLevel.GameMaster && m_Tournament != null )
			{
				...
				if ( m_Tournament.CurrentStage == TournamentStage.Inactive )
					...
					list.Add( new CancelEntry( from, m_Tournament ) );
			}
		}
		...
		private class CancelEntry : ContextMenuEntry
		{
			private Tournament m_Tournament;
			private Mobile m_From;

			public CancelEntry( Mobile from, Tournament tourny ) : base( 5114 )
			{
				m_Tournament = tourny;
				m_From = from;
			}
			...
			public override void OnClick()
			{
				if ( m_Tournament.Stage != TournamentStage.Inactive )
				{
					...
					for ( int i = 0; i < m_Tournament.Participants.Count; ++i )
					{
						...
						{
							PlayerMobile pm = part.Players[j] as PlayerMobile;

							dc = pm.DuelContext;
	
							if ( pm != null && pm.DuelPlayer != null )
								pm.DuelPlayer.Eliminated = true;
							
							if ( pm != null && dc != null && dc.Arena != null )
							{
								dr = dc;
								//pm.MoveToWorld( dc.Arena.Outside, dc.Arena.Facet ); 
								dc.SendOutside( pm );
								pm.LocalOverheadMessage( MessageType.Regular, 0x22, false, "The event has been cancelled." );
								pm.DuelPlayer = null;
							}
						}
					}

					m_Tournament.Undefeated.Clear();
					m_Tournament.Participants.Clear();
					m_Tournament.Pyramid.Levels.Clear();
					...
					m_Tournament.Stage = TournamentStage.Inactive;
					m_Tournament.Alert( "Hear ye! Hear ye!", String.Format( "The tournament has been cancelled by {0}.", m_From.Name ) );
					m_Tournament = null;
				}
			}
		}
	...
	public class Tournament
	{
		...
		{
			return m_Participants.Count * m_PlayersPerParticipant * 1000;
		}
		...
		public TournamentBracketGump( Mobile from, Tournament tourny, TournyBracketGumpType type, ArrayList list, int page, object obj ) : base( 50, 50 )
		{
			...
			switch ( type )
			{
				...
				case TournyBracketGumpType.Player_Info:
				{
					...
					AddHtml( 25, 113, 250, 20, String.Format( "Level: {0}", Ladder.GetLevel( entry.Experience ) ), false, false );
					AddHtml( 25, 133, 250, 20, String.Format( "Wins: {0:N0}", entry.Wins ), false, false );
					AddHtml( 25, 153, 250, 20, String.Format( "Losses: {0:N0}", entry.Losses ), false, false );
