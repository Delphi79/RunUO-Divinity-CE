...
namespace Server.Engines.MyRunUO
{
	public class MyRunUO : Timer
	{
		...
		public void InsertMobile( Mobile mob )
		{
            ...
            int ladderWins = 0, ladderLosses = 0, ladderExp = 0;
            LadderEntry entry = null;
            if ( Ladder.Instance != null )
                entry = Ladder.Instance.FindNoCreate(mob);
            if (entry != null)
            {
                ladderWins = entry.Wins;
                ladderLosses = entry.Losses;
                ladderExp = entry.Experience;
            }
			...
			if ( Config.LoadDataInFile )
			{
                ...
                if ( mob is PlayerMobile )
                    m_OpMobiles.Write( ((PlayerMobile)mob).NewbKills );
                else
                    m_OpMobiles.Write( "0" );
                m_OpMobiles.Write( EntrySep );
				...
				m_OpMobiles.Write( 0 );
				m_OpMobiles.Write( EntrySep );
				m_OpMobiles.Write( "NULL" );
				m_OpMobiles.Write( EntrySep );
				m_OpMobiles.Write( ladderExp );
				m_OpMobiles.Write( EntrySep );
				m_OpMobiles.Write( ladderWins );
				m_OpMobiles.Write( EntrySep );
				m_OpMobiles.Write( ladderLosses );
