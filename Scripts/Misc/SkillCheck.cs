...
namespace Server.Misc
{
	public class SkillCheck
	{
        ...
        private static double PVP_GainFactor = 2.0;
		private static double CRAFT_GainFactor = 0.5;
		...
		public static void Initialize()
		{
            ...
            // Toughen up some skills
            SkillInfo.Table[(int)SkillName.Alchemy].GainFactor = CRAFT_GainFactor;
            SkillInfo.Table[(int)SkillName.AnimalTaming].GainFactor = CRAFT_GainFactor;
            SkillInfo.Table[(int)SkillName.Blacksmith].GainFactor = CRAFT_GainFactor;
            SkillInfo.Table[(int)SkillName.Carpentry].GainFactor = CRAFT_GainFactor;
			SkillInfo.Table[(int)SkillName.Fletching].GainFactor = CRAFT_GainFactor;
			SkillInfo.Table[(int)SkillName.Inscribe].GainFactor = CRAFT_GainFactor;
            SkillInfo.Table[(int)SkillName.Tinkering].GainFactor = CRAFT_GainFactor;
            SkillInfo.Table[(int)SkillName.Tailoring].GainFactor = CRAFT_GainFactor;
            ...
            SkillInfo.Table[(int)SkillName.Meditation].IntGain += 0.5;

            SkillInfo.Table[(int)SkillName.Magery].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.MagicResist].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.Meditation].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.EvalInt].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.Anatomy].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.Tactics].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.Wrestling].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.Swords].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.Macing].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.Fencing].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.Healing].GainFactor = PVP_GainFactor;
            SkillInfo.Table[(int)SkillName.Parry].GainFactor = PVP_GainFactor;
		}
        ...
        public static bool CheckSkill(Mobile from, Skill skill, object amObj, double chance)
        {
            ...
            // hunger bonus: completely full +2% , completely empty -0.5%
            double bonus = (from.Hunger - 4) / 800.0; // hunger goes from 20 (full) to 0 (empty)
            if (bonus > 0.02)
                bonus = 0.02;
            else if (bonus < 0)
                bonus = 0;
            ...
            bool success = ((chance + bonus) >= Utility.RandomDouble());
            ...
            gc += bonus; // include the bonus as a skill gain bonus too.

            gc *= skill.Info.GainFactor;

            gc /= 1.8; //was 2.5 // was 2.0
            ...
            if (from.AccessLevel == AccessLevel.Administrator)
                from.SendMessage("Gain chance: {0}%", gc * 100);
		...
		{
            if (Region.Find(from.Location, from.Map).IsPartOf(typeof(Engines.ConPVP.SafeZone)))
                return false;
		...
		{
			if ( from.Region.IsPartOf(typeof(Regions.Jail)) || from.Region.IsPartOf(typeof(Engines.ConPVP.SafeZone)) )
				return;
		...
		private static TimeSpan m_StatGainDelay = TimeSpan.FromMinutes( 7.0 );
