...
namespace Server.Guilds
{
	...
	public class Guild : BaseGuild
	{
        ...
        private static Dictionary<System.Net.IPAddress, GuildType> _IPTable = new Dictionary<System.Net.IPAddress, GuildType>();

        public static GuildType GetGuildType(Mobile m)
        {
            if (m.NetState != null && _IPTable.ContainsKey(m.NetState.Address))
            {
                GuildType type = _IPTable[m.NetState.Address];

                if (type != GuildType.Regular)
                    return type;
            }

            if (m.Guild != null && m.Guild.Type != GuildType.Regular)
            {
                if (m.NetState != null)
                {
                    if (_IPTable.ContainsKey(m.NetState.Address))
                        _IPTable[m.NetState.Address] = m.Guild.Type;
                    else
                        _IPTable.Add(m.NetState.Address, m.Guild.Type);
                }
            ...
            for (int i = 0; i < a.Length; ++i)
            {
                if (a[i] != null)
                {
                    if (a[i].Guild != null && a[i].Guild.Type != GuildType.Regular)
                    {
                        if ( m.NetState != null )
                            _IPTable.Add(m.NetState.Address, a[i].Guild.Type);
                        return a[i].Guild.Type;
                    }
                }
            }
		...
		public void GuildChat( Mobile from, int hue, string text )
		{
            ...
            text = String.Format( "({0}): {1}", from.Name, text );
			...
			for( int i = 0; i < m_Members.Count; i++ )
			{
				...
				{
					if( p == null )
						p = Packet.Acquire( new AsciiMessage( (Serial)0x00000001, -1, MessageType.Regular, hue, 3, from.Name, text ) );
