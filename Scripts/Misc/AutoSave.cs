	...
	public class AutoSave : Timer
	{
		...
		protected override void OnTick()
		{
			...
			else
			{
				...
				/*if ( m > 0 && s > 0 )
					World.Broadcast( 0x34, true, "The world will save in {0} minute{1} and {2} second{3}.", m, m != 1 ? "s" : "", s, s != 1 ? "s" : "" );
				else if ( m > 0 )
					World.Broadcast( 0x34, true, "The world will save in {0} minute{1}.", m, m != 1 ? "s" : "" );
				else
                    World.Broadcast( 0x34, true, "The world will save in {0} second{1}.", s, s != 1 ? "s" : "" );*/

                World.Broadcast(0x34, true, "Server data saving. You may experience some lag for the next several seconds.");
		...
		public static void Save()
		{
			...
			World.Save( false );
            World.Broadcast(0x34, true, "World save complete.");
		}
