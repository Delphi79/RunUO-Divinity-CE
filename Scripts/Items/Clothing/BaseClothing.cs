...
namespace Server.Items
{
	...
	public abstract class BaseClothing : Item, IDyable, IScissorable, IFactionItem, ICraftable, IWearableDurability
	{
		...
		public override void OnRemoved( object parent )
		{
            ...
            {
/*
                if ( m_FactionState != null && ( Hue == Server.Battle.PlayerCommands.ChaosItemHue || Hue == Server.Battle.PlayerCommands.OrderItemHue ) )
                {
                    Hue = 0;
                }
*/
		...
		public virtual int OnHit( BaseWeapon weapon, int damageTaken )
		{
			...
			if ( Utility.Random( 100 ) < 25 && LootType != LootType.Blessed ) // 25% chance to lower durability
			{
		...
		public virtual bool Scissor( Mobile from, Scissors scissors )
		{
			...
			if ( item != null && item.Ressources.Count == 1 && item.Ressources.GetAt( 0 ).Amount >= 2 )
			{
				try
				{
					...
					ScissorHelper( from, res, m_PlayerConstructed ? (item.Ressources.GetAt( 0 ).Amount / 2) : 1, !(res is BaseLeather) );
