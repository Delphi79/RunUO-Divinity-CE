...
namespace Server.Items
{
	...
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability
	{
		...
		public virtual int DiceDamage{ get{ return -1; } }
		...
		public override void OnRemoved( object parent )
		{
			if ( parent is Mobile )
			{
                if (Hue == Server.Battle.PlayerCommands.ChaosItemHue || Hue == Server.Battle.PlayerCommands.OrderItemHue)
                {
                    Hue = 0;
                }
                
				Mobile m = (Mobile)parent;
				BaseWeapon newWeapon = m.Weapon as BaseWeapon;
                ...
                TimeSpan delay = GetDelay( m );
                if ( m.NextCombatTime >= (DateTime.MinValue+delay) )
                    m.NextCombatTime -= delay; // forget the delay for their old weapon (this weapon)
                if ( newWeapon != null )
                    m.NextCombatTime += newWeapon.GetDelay( m ); // use the delay for their current weapon
		...
		public virtual bool CheckHit( Mobile attacker, Mobile defender )
		{
			...
			//double chance = ourValue / (theirValue * 2.0); // about 50% to hit someone with = skill
			double chance = ourValue / (theirValue * 1.85); // about 54% to hit someone with = skill
		...
		public virtual TimeSpan GetDelay( Mobile m )
		{
			...
			{
				int v = (m.Stam + 100) * speed; // dex? stam? what.
		...
		public virtual int AbsorbDamage( Mobile attacker, Mobile defender, int damage )
		{
			...
			int virtualArmor = defender.VirtualArmor;
			int virtualMod = defender.VirtualArmorMod;

			// natural armor for cloth for players
			if ( defender.Player && virtualArmor < 16 )
				virtualArmor = 16;
		...
		public virtual double GetBaseDamage( Mobile attacker )
		{
			...
			if ( attacker is BaseCreature || ( damage = DiceDamage ) == -1 )
			{
				int min, max;
		...
		{
            if ( m_MaxHits > 0 )
                return (int)Math.Round( ((double)damage) * ( ( (double)m_Hits / (double)m_MaxHits ) * 0.26 + 0.75 ) ); // up to +1% bonus / -25% penalty
            else
                return damage;
		}
        ...
        public bool IsMagic
        {
            get
            {
                return m_Slayer != SlayerName.None || m_DurabilityLevel != WeaponDurabilityLevel.Regular || m_DamageLevel != WeaponDamageLevel.Regular || m_AccuracyLevel != WeaponAccuracyLevel.Regular || (m_Effect != SpellEffect.None && m_EffectCharges > 0);
            }
        }
        ...
        public override string BuildSingleClick()
        {
            ...
            if (Identified)
            {
                ...
                if ( m_Slayer != SlayerName.None )
                {
                    ...
                    string strEnt = null;
                    if (entry != null)
                        strEnt = Core.CliLoc.Table[entry.Title] as string;

                    if ( strEnt != null )
                        sb.Append( strEnt.ToLower() );
                    else
                        sb.Append( "enchanted" );
                    sb.Append(", ");
                }
        ...
        public int SpellCharges
        {
            ...
            set { m_EffectCharges = value; InvalidateProperties(); }
        }
