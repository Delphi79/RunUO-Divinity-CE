...
namespace Server.Items
{
	public class Corpse : Container, ICarvable
	{
        ...
        public static TimeSpan PlayerLootRightSacrifice
        {
		...
		public static string GetCorpseName( Mobile m )
		{
            if ( m is BaseCreature )
                return ((BaseCreature)m).CorpseName;
		...
		public override void OnItemLifted( Mobile from, Item item )
		{
			...
			if ( item != this && IsCriminalAction( from ) )
				Timer.DelayCall( TimeSpan.Zero, new TimerStateCallback<Mobile>( ExecuteCriminalAction ), from );
		...
		public void Carve( Mobile from, Item item )
		{
			...
			else if ( ((Body)Amount).IsHuman && ItemID == 0x2006 )
			{
				...
				new Torso( dead ).MoveToWorld( Location, Map );
				new LeftLeg( dead ).MoveToWorld( Location, Map );
				new LeftArm( dead ).MoveToWorld( Location, Map );
				new RightLeg( dead ).MoveToWorld( Location, Map );
				new RightArm( dead ).MoveToWorld( Location, Map );
