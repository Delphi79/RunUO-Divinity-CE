...
namespace Server.Items
{
	public abstract class BaseArmor : Item, IScissorable, IFactionItem, ICraftable, IWearableDurability
	{
		...
		public virtual double BaseArmorRatingScaled
		{
			get
			{
        ...
        /*FUCK YOU MOTHER FUCKING UO PIECE OF SHIT WHORE
		public virtual double ArmorRating
		{
			get
			{
				...
				switch ( m_Resource )
				{
					...
					case CraftResource.SpinedLeather:	ar += 1; break;
					case CraftResource.HornedLeather:	ar += 2; break;
					case CraftResource.BarbedLeather:	ar += 3; break;
				}
        ...
        public virtual double ArmorRatingScaled
        {
            get
            {
		...
		{
			double HalfAr = UnscaledArmorRating / 2.0;

			// natural armor for players // 7.35 75% - 7.8 100%
			if ( HalfAr < 7.80 )
				HalfAr = 7.80;
			
			int Absorbed = (int)(HalfAr + HalfAr*Utility.RandomDouble());

			//additional asborbtion from armor 
			...
			if ( m_MaxHitPoints > 0 && 20 > Utility.Random( 100 ) ) // 20% chance to lower durability // was 50%
			{
				...
				else
				{
					...
					if ( wear > 0 )
					{
                        if ( m_HitPoints > wear && m_MaxHitPoints > 1 )
                        {
                            HitPoints -= wear;

                            if ( m_HitPoints < 10 || ((float)m_HitPoints / m_MaxHitPoints) <= 0.1 )
                            {
                                --MaxHitPoints;
