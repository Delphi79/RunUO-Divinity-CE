...
namespace Server.Items
{
	public abstract class BaseExplosionPotion : BasePotion
	{
        ...
        private static int GetExplodeDelay()
        {
            return Utility.RandomMinMax( 3, 8 );
        }
		...
		public override void Drink( Mobile from )
		{
			...
			if ( m_Timer == null )
			{
                int delay = GetExplodeDelay();

				from.SendLocalizedMessage( 500236 ); // You should throw it now!
				m_Timer = Timer.DelayCall( TimeSpan.FromSeconds( 0.75 ), TimeSpan.FromSeconds( 1.0 ), delay+1, new TimerStateCallback( Detonate_OnTick ), new object[]{ from, delay } );
			}
		}
