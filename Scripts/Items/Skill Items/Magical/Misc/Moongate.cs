...
namespace Server.Items
{
	...
	public class Moongate : Item
	{
		...
		{
			if ( IsInTown( from.Location, from.Map ) && 
                ( !IsInTown( m_Target, m_TargetMap ) || 
                IsHostileVirtueArea( m_Target, m_TargetMap, from ) ||
                (from.Map != Map.Felucca && TargetMap == Map.Felucca && ShowFeluccaWarning) ) )
			{
                ...
                //new MoongateConfirm( from, this ).SendTo( from.NetState );
			}
			else
			{
		...
		private class DelayTimer : Timer
		{
			...
			public DelayTimer( Mobile from, Moongate gate, int range ) : base( TimeSpan.FromSeconds( 0.5 ) )
			{
				m_From = from;
				m_Gate = gate;
				m_Range = range;
    ...
    /*public class MoongateConfirm : Server.Menus.Questions.QuestionMenu
    {
        ...
        public MoongateConfirm( Mobile from, Moongate gate )
