...
namespace Server.Regions
{
	public class HouseRegion : BaseRegion
	{
		...
		public override void OnLocationChanged( Mobile m, Point3D oldLocation )
		{
			...
			// we want people to be able to freely enter private custom houses
			else if ( m_House.IsAosRules && !m_House.Public && m_House.IsBanned( m ) && m_House.IsInside( m ) )
			{
				m.Location = m_House.BanLocation;
	...
	/*			We want to be able to ban people from custom houses
	...
	*/
			...
			else if ( e.HasKeyword( 0x27 ) ) // I wish to place a strongbox
			{
                ...
                from.SendMessage( "You may not place a strongbox." );
			}
