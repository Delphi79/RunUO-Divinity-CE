...
namespace Server.Regions
{
	public class GuardedRegion : BaseRegion
	{
        ...
        public virtual Guilds.GuildType GetVirtueOwner()
        {
            return Guilds.GuildType.Regular;
        }
		...
		public void CheckGuardCandidate( Mobile m )
		{
			...
			if ( IsGuardCandidate( m ) )
			{
				...
				if ( timer == null )
				{
					timer = new GuardTimer( m, m_GuardCandidates, IsConstantCandidate( m ) );
					timer.Start();
		...
		public void CallGuards( Point3D p )
		{
			...
			{
				if ( IsGuardCandidate( m ) && ( ( IsConstantCandidate( m ) && m.Region.IsPartOf(this) ) || m_GuardCandidates.ContainsKey( m ) ) )
				{
					GuardTimer timer = null;
					m_GuardCandidates.TryGetValue( m, out timer );
					...
					if ( m.Region != null && this == m.Region )
						MakeGuard( m );
		...
		public virtual bool IsGuardCandidate( Mobile m )
		{
			if ( m is BaseGuard || !m.Alive || m.AccessLevel > AccessLevel.Player || m.Blessed || IsDisabled() )
				return false;
		...
		private class GuardTimer : Timer
		{
			...
			public GuardTimer( Mobile m, Dictionary<Mobile, GuardTimer> table, bool constant ) : base( TimeSpan.FromSeconds( 15.0 ) )
			{
    ...
    public class VirtueGuardedRegion : TownRegion
    {
        private Battle.BattleController _Battle;

        public VirtueGuardedRegion(string name, Map map, int priority, params Rectangle3D[] area)
            : base(name, map, priority, area)
        {
        }

        public VirtueGuardedRegion(string name, Map map, int priority, params Rectangle2D[] area)
            : base(name, map, priority, area)
        {
        }

        public VirtueGuardedRegion(XmlElement xml, Map map, Region parent)
            : base(xml, map, parent)
        {
        }

        public Battle.BattleController Battle
        {
            get { return _Battle; }
            set { _Battle = value; }
        }

        public override Guilds.GuildType GetVirtueOwner()
        {
            if (_Battle != null && !_Battle.Deleted)
                return _Battle.GetGuardType();
            else
                return base.GetVirtueOwner();
        }
        ...
        public override bool AllowBeneficial(Mobile from, Mobile target)
        {
            if (_Battle != null && !_Battle.Deleted)
            {
                if (!_Battle.AllowBeneficialAction(from, target))
                    return false;
            }
        ...
        public override bool AllowHarmful(Mobile from, Mobile target)
        {
            if (_Battle != null && !_Battle.Deleted)
            {
                if (!_Battle.AllowHarmfulAction(from, target))
                    return false;
            }
