 ...
 *
 *   $Id: Item.cs 239 2007-09-13 09:19:23Z asayre $
 *
 ***************************************************************************/
...
namespace Server
{
	...
	public class Item : IEntity, IHued, IComparable<Item>, ISerializable
	{
		...
		{
            SingleClickChanged(); // divinity hax
        ...
        {
            AppendClickName(sb, Amount != 1);
        }

        public virtual void AppendClickName(StringBuilder sb, bool plural)
        {
        ...
        public virtual string BuildSingleClick()
        {
            ...
            if (plural && ItemID != 0x2006)
                sb.AppendFormat("{0} ", Amount);

            if (AppendLootType(sb))
                sb.Append(" ");
            AppendClickName(sb, plural);
            InsertNamePrefix(sb);
        ...
        public virtual bool ForceClickLabelNumber
        {
            get
            {
                return (LabelNumber != (1020000 + (ItemID & 0x3FFF)));
            }
        }
		...
		public virtual void OnSingleClick( Mobile from )
		{
			...
			if ( ns != null )
			{
                if (ForceClickLabelNumber && (Name == null || Name.Length <= 0))
                {
                    // default code: (do this if it has a non standard labelnumber)
                    if (DisplayLootType)
                ...
                {
                    // special divinity code for old skewl names:
